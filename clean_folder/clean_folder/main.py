...
def scan_folder(path_to_folder: Path) -> None:  # Функция сканирует папку на наличие файлов и вложенных папок
    for element in path_to_folder.iterdir():
        if element.is_dir() and element in EXTANSIONS.keys():
            continue
        elif element.is_dir():
            if any(element.iterdir()):
                scan_folder(element)  # Рекурсивно проходим по всем вложенным папкам
            else:
                element.rmdir()  # Удаляем пустые папки
        else:
            sort_file(element)
            handle_file(element)
            if not any(element.parent.iterdir()): element.parent.rmdir()  # Раздаем задачи другим функциям


def normalize_name(path_to_file: Path) -> str:  # Траслитирация названий файлов
...
def unpack_archive(path_to_archive: Path, folder: str) -> None:  # Распаковка архивов
    ...
    Path.unlink(path_to_archive)  # Удаление архива после распаковки
...
def handle_file(path_to_file: Path) -> None:  # Создание списков по группам файлов
...
def sort_file(path_to_file: Path) -> None:  # Сортировка файлов по расширениям и типам
    ext = get_ext(path_to_file)
    for key, value in EXTANSIONS.items():
        if ext in value:
            KN_EXT.add(ext)
...
def move_file(path_to_file: Path, folder: str) -> None:  # Перенос файлов в папки, соответствующие содержанию
    new_folder = path_to_folder / folder
    if not new_folder.exists():
        new_folder.mkdir(exist_ok=True, parents=True)
    try:
